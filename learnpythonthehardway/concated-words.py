...
class Solution(object):
    ...
    def findAllConcatenatedWordsInADict2(self, words):
        ...
        for w in words:
            if not w:
                continue
            if self.dfs2(dict, w, 0, 0):
                res.append(w)

        return res

    def dfs2(self, dict, word, pos, cnt):
        if pos >= len(word) and cnt > 1:
            return True
        for i in xrange(pos + 1, len(word) + 1):
            w = word[pos:i]
            if w in dict and self.dfs2(dict, word, i, cnt + 1):
                return True
...
if __name__ == '__main__':
    print Solution().findAllConcatenatedWordsInADict2(
