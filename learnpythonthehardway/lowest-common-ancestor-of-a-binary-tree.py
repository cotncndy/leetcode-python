...
class Solution:
    ...
    # review this algorithm , so concise
    def lowestCommonAncestor(self, root, p, q):
        if root in (None, p, q):
            return root

        left, right = [self.lowestCommonAncestor(child, p, q) for \
                       child in (root.left, root.right)]  # knowledge that's really cool
...
if __name__ == '__main__':
    r = TreeNode(1)
    r1, r2 = TreeNode(2), TreeNode(3)
    r.left, r.right = r1, r2
    r1.left, r1.right = TreeNode(4), TreeNode(5)
    r2.right = TreeNode(8)

    print Solution().lowestCommonAncestor(r, r2, r2.right)
