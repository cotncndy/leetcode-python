...
# If the file content is very large (GB level), how will you modify your solution?  # knowledge md5 or sha hash ?
...
# How to make sure the duplicated files you find are not false positive?  #knowledge bloom filter?
import collections
...
class Solution(object):
    def findDuplicate(self, paths):
        ...
        def stripfileNameAndContent(s):
            idx1, idx2 = s.index('('), s.index(')')
            return (s[:idx1], s[idx1+1:idx2])

        m = collections.defaultdict(set)
        for p in paths:
            files = p.split()
            for f in files[1:]:
                name, content = stripfileNameAndContent(f)
                m[content].add(files[0] + '/' + name)
