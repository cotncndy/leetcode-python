...
class NumMatrix(object):
    def __init__(self, matrix):
        ...
        self._sums = [[0 for _ in xrange(n + 1)] for _ in xrange(m + 1)]
        for i in xrange(1, m + 1):
            for j in xrange(1, n + 1):
                self._sums[i][j] = self._sums[i - 1][j] + self._sums[i][j - 1] - self._sums[i - 1][j - 1] + \
                                   matrix[i - 1][j - 1]
    ...
    def sumRegion(self, row1, col1, row2, col2):
        ...
        :rtype: int
        """
        return self._sums[row2 + 1][col2 + 1] - self._sums[row1][col2 + 1] - self._sums[row2 + 1][col1] + \
