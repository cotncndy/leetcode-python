    ...
    def validIPAddress(self, IP):
        ...
        if len(tokens) == 4:
            for i in xrange(len(tokens)):
                if not tokens[i].isdigit() or not 0 <= int(tokens[i]) < 256 \
                        or (tokens[i][0] == '0' and len(tokens[i]) > 1):
                    return "Neither"
        ...
        if len(tokens) == 8:
            for i in xrange(len(tokens)):
                if not (1 <= len(tokens[i]) <= 4) or \
                        not all(c in string.hexdigits for c in tokens[i]):
                    return "Neither"
